...
Board::Board() {
  ...
  for (int rows = 0; rows < 5; rows++) {
    for (int columns = 0; columns < 5; columns++) {
      vertex_array[rows][columns] = vf(rows * block_size, columns * block_size);
    }
  }
  ...
  for (int rows = 0; rows < 5; rows++) {
    for (int columns = 0; columns < 5; columns++) {
      if (elements_array[rows][columns] == 2)
        baaghs.push_back(Baagh(columns * block_size, rows * block_size));
    }
  }
  is_game_over = false;
}
...
void Board::draw(sf::RenderTarget &target, sf::RenderStates states) const {
  ...
  states.transform.translate(vf(90, 60));

  DrawBoard(target, states);
  sf::RenderStates new_state;
  new_state.transform *= getTransform();
  new_state.transform.translate(sf::Vector2f(1000, 60));
  ...
  for (auto &baakhra : baakhras) {
    ...
    if (elements_array[(int)(pos.y / block_size)][(int)(pos.x / block_size)] ==
        1)
    ...
    if (count % 2) {
      ...
      if (baakhras.size() < 20) {
        if (sf::Mouse::isButtonPressed(sf::Mouse::Button::Left)) {
          ...
          for (int rows = 0; rows < 5; rows++) {
            for (int columns = 0; columns < 5; columns++) {
              if (abs(mouse_position.x - columns * block_size) <= 50 &&
                  abs(mouse_position.y - rows * block_size) <= 50) {
                if (elements_array[rows][columns] == 0) {
                  baakhras.push_back(
                      Baakhra(columns * block_size, rows * block_size));
        ...
        for (auto &baakhra : baakhras)
          if (!Baakhra::completed)
            baakhra.ProcessEvents(m_window, elements_array, block_size);
        if (Baakhra::completed)
          count++;
      }
    }
    ...
    for (int columns = 0; columns < 5; columns++) {
      if (elements_array[rows][columns] == 1) {
        ...
        if ((rows + columns) % 2 == 0) {
          ...
          for (int i = -1; i < 2; i++) {
            for (int j = -1; j < 2; j++) {
              if ((rows + i) >= 0 && (columns + j) >= 0 && (rows + i) <= 4 &&
                  (columns + j) <= 4) {
                if ((abs(i) + abs(j)) > 0) {
                  cnt++;
                  if (elements_array[rows + i][columns + j] >= 1) {
                    b_cnt++;
                  }
                  if ((rows + 2 * i) >= 0 && (columns + 2 * j) >= 0 &&
                      (rows + 2 * i) <= 4 && (columns + 2 * j) <= 4) {
                    if (elements_array[rows + 2 * i][columns + 2 * j] == 0)
                      can_eat = true;
                  }
                }
              }
...
void Board::DrawText(sf::RenderTarget &target, sf::RenderStates &states) const {
  ...
  if (is_game_over) {
    ...
    if ((baakhras.size() - baakhra_count) > 4)
      str = "La Badhai chha Baagh.";
    else {
      str = "La Badhai chha Baakhra";
    }
